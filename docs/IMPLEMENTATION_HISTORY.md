...
| 三维 quad 与双眼投影/标定 | 完成工程阶段门 | 独立校准页、真实 3840×1080 校准图与 host 数学测试；佩戴者主观校准尚待记录 |
...
下一步：先由实际佩戴者完成双眼校准，记录保存后的 profile、重新佩戴的一致性及舒适融合结果；该门通过后再建立可验证的 Air 4 Pro 头部姿态源并驱动每帧相机。随后把 pointer → ray → 最近窗口 → 局部内容坐标 → display-aware 注入接入运行时，最后执行一次 3840×1080、四窗口、头动、射线操作和异常释放的联合真机验收。
